...
const {
    buildHookPluginConsolePrefix,
    extractHookPluginConsoleLog,
} = require("../electron/hookPluginConsole.ts");
...
test("启用插件组合脚本会自动包装 console 前缀", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "wmpf-hook-manager-"));
    try {
        const hookManager = new HookManager({ configDir: tempDir });
        await hookManager.load();

        const plugin = await hookManager.addPlugin("日志插件", "console.log('hello', 123);", "inline");
        await hookManager.togglePlugin(plugin.id, true);

        const combined = hookManager.getEnabledScripts();
        assert.match(combined, /const console = \{/);
        assert.match(combined, /\[HookPlugin:日志插件\]/);
        assert.match(combined, /console\.log\('hello', 123\);/);
    } finally {
        await fs.rm(tempDir, { recursive: true, force: true });
    }
});

test("Runtime.consoleAPICalled 可提取为插件日志", () => {
    const payload = JSON.stringify({
        method: "Runtime.consoleAPICalled",
        params: {
            type: "warning",
            args: [
                { type: "string", value: buildHookPluginConsolePrefix("日志插件") },
                { type: "string", value: "tick slow" },
                { type: "number", value: 42 },
            ],
            stackTrace: {
                callFrames: [
                    { url: "wmpf-debugger-hook-abc123.js" },
                ],
            },
        },
    });

    assert.deepEqual(extractHookPluginConsoleLog(payload), {
        level: "warn",
        message: "[HookPlugin:日志插件] tick slow 42",
    });
});
